...
const shortNames: Record<string, string> = {
  ...
  mui: "materialui",
  md: "markdown",
  copilot: "microsoft365copilot",
  microsoftcopilot: "microsoft365copilot",
...
router.get("/icons", async (req: Request, res: Response) => {
  ...
  if (i && typeof i === "string") {
    ...
    const fullIconsList = iconsList.map(
      (icon) => shortNames[icon.trim()] || icon.trim()
    );
    const icons: string[] = [];
    for (const icon of fullIconsList) {
      ...
      try {
        ...
        let radiusValue = Math.min(
          Math.max(Number(radius) || minRadius, minRadius),
          maxRadius
        );
    ...
    sendError(
      ...
      400,
      "Bad Request",
      "The request couldn't be understood or was missing required parameters."
    );
  }
});
...
router.get("/icons/all", async (req: Request, res: Response) => {
  const iconsDir = path.join(__dirname, "../../icons");
  const baseUrl =
    "https://raw.githubusercontent.com/syvixor/skills-icons/refs/heads/main/icons";
